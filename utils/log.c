...
void        log_output_bin(unsigned char level, const char *tag, const char *file, const char *func, int line, const char *msg, unsigned char *dat, uint16_t dataLen)
{
    ...
    log_printf(color_output_info[ level ]);
#endif
...
#if LOG_COLOR_ENABLE
    /* add CSI start sign and color info */
    length += sprintf(rt_log_buf + length, CSI_START);
    length += sprintf(rt_log_buf + length, color_output_info[ level ]);
#endif
