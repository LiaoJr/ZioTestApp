...
static int socket_connect (const char *host, int port, const char *interface_name) {
  ...
  if(h == NULL) {
    IOT_ERROR("gethostbyname failed: %s" ENDL, host);
    return -1;
  }
  ...
  IOT_INFO("connecting... " AW_PRINT_IP4ADDR_FMT ":%u" ENDL, AW_PRINT_IP4ADDR_VAL(s_in.sin_addr.s_addr), ntohs(s_in.sin_port));
  if((sock = (int) socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0) {
    IOT_ERROR("socket error" ENDL);
    return -1;
  }
  ...
  if(connected != 0) {
    if(errno != EINPROGRESS) {
      IOT_ERROR("connect errno :%d != %d" ENDL, errno, EINPROGRESS);
      goto error_exit;
    } else {
      ...
      if(res < 0) {
        IOT_ERROR("network error in connect" ENDL);
        goto error_exit;
      } else if(res == 0) {
        IOT_ERROR("connect time out" ENDL);
        goto error_exit;
      } else if(1 == res) {
        if(FD_ISSET(sock, &wset)) {
          IOT_INFO("connect succeed." ENDL);
          fcntl(sock, F_SETFL, fcntl(sock, F_GETFL, 0) & ~O_NONBLOCK);
        } else {
          IOT_ERROR("other error when select: errno=%d" ENDL, errno);
          goto error_exit;
        }
      }
    }
...
#else
    int ret = setsockopt(s->sock, SOL_SOCKET, SO_RCVTIMEO, &timeout_ms, sizeof(timeout_ms));
#endif
    IOT_INFO("ret=%d" ENDL, ret);
...
static IoT_Error_t iot_socket_read(Network *pNetwork, unsigned char *pMsg, size_t len, Timer *timer, size_t *read_len) {
  ...
  while(len > 0) {
    ...
    if(ret > 0) {
      ...
      pMsg += ret;
      len -= ret;
    } else if((ret == -1 || ret == 0) && !(errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK)) {
      return NETWORK_SSL_READ_ERROR;
    } 
