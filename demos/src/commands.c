...
#define STR_NAME_MYZGGW_MOVE_NODE "move_node"
#define STR_NAME_MYZGGW_SET_PAN_CHANNEL "set_pan_channel"
#define STR_NAME_MYZGGW_MANAGE_WHITELIST "manage_white_list"
#define STR_NAME_MYZGGW_GET_WHITELIST "get_white_list"
#define STR_NAME_MYZGGW_SET_WHITELIST_ENABLE "whitelist_enable_exec"

#define STR_KEY_NEW_GW_DEV_TYPE "newGwDevType"
#define STR_KEY_NEW_GW_DEV_ID   "newGwDevID"
#define STR_KEY_NEW_PAN_ID      "newPANID"
#define STR_KEY_NEW_CHANNEL_ID  "newChannelID"
#define STR_KEY_GROUP_ID        "groupID"
#define STR_KEY_PAN_ID      "PANID"
#define STR_KEY_CHANNEL_ID  "ChannelID"
...
static BOOL myzggw_move_node(command_t* cmd, struct _client_t* client, params_t* params) {
	int cmdid = 0;
	BOOL ret = TRUE;
	const char* message = "ok";
	const char* new_gw_dev_type = NULL;
	const char* new_gw_dev_iD = NULL;
	int new_pan_id = 0;
	int new_channel_id = 0;

	goto_done_if_fail(cmd != NULL && client != NULL && params != NULL, BAD_PARAMETERS);
	goto_done_if_fail(params_get_string(params, STR_KEY_NEW_GW_DEV_TYPE, &new_gw_dev_type), BAD_PARAMETERS);
	goto_done_if_fail(params_get_string(params, STR_KEY_NEW_GW_DEV_ID, &new_gw_dev_iD), BAD_PARAMETERS);
	goto_done_if_fail(params_get_int(params, STR_KEY_NEW_PAN_ID, &new_pan_id), BAD_PARAMETERS);
	goto_done_if_fail(params_get_int(params, STR_KEY_NEW_CHANNEL_ID, &new_channel_id), BAD_PARAMETERS);
	printf("newGwDevType: %s, newGwDevID: %s, newPANID: %d, newChannelID: %d\n", new_gw_dev_type, new_gw_dev_iD, new_pan_id, new_channel_id);
	(void)cmd;
	printf("%s\n", __func__);
	params_dump(params);
	/*TODO: implement the command*/
...
static BOOL myzggw_set_pan_channel(command_t* cmd, struct _client_t* client, params_t* params) {
	int cmdid = 0;
	BOOL ret = TRUE;
	const char* message = "ok";
	int pan_id = 0;
	...
	goto_done_if_fail(cmd != NULL && client != NULL && params != NULL, BAD_PARAMETERS);
	goto_done_if_fail(params_get_int(params, STR_KEY_PAN_ID, &pan_id), BAD_PARAMETERS);
	goto_done_if_fail(params_get_int(params, STR_KEY_CHANNEL_ID, &channel_id), BAD_PARAMETERS);
	printf("PANID: %d, ChannelID: %d\n", pan_id, channel_id);
	(void)cmd;
	printf("%s\n", __func__);
	params_dump(params);
	/*TODO: implement the command*/
...
static BOOL myzggw_manage_whitelist_exec(command_t* cmd, struct _client_t* client, params_t* params) {
	int cmdid = 0;
	BOOL ret = TRUE;
	const char* message = "ok";
	int action = 0;
	const char* subdevtype = NULL;
	const char* subdevid = NULL;

	goto_done_if_fail(cmd != NULL && client != NULL && params != NULL, BAD_PARAMETERS);
	goto_done_if_fail(params_get_int(params, "action", &action), BAD_PARAMETERS);
	goto_done_if_fail(params_get_string(params, "subdevtype", &subdevtype), BAD_PARAMETERS);
	goto_done_if_fail(params_get_string(params, "subdevid", &subdevid), BAD_PARAMETERS);

	printf("action: %d, subdevtype: %s, subdevid: %s\n", action, subdevtype, subdevid);
...
static BOOL myzggw_get_whitelist_exec(command_t* cmd, struct _client_t* client, params_t* params) {
	int cmdid = 0;
	BOOL ret = TRUE;
	const char* message = "ok";
...
static BOOL myzggw_whitelist_enable_exec(command_t* cmd, struct _client_t* client, params_t* params) {
	int cmdid = 0;
	BOOL ret = TRUE;
	const char* message = "ok";
	int whiteListEnble = 0;

	goto_done_if_fail(cmd != NULL && client != NULL && params != NULL, BAD_PARAMETERS);
	goto_done_if_fail(params_get_int(params, "whiteListEnble", &whiteListEnble), BAD_PARAMETERS);
	printf("whiteListEnble: %d\n", whiteListEnble);
	(void)cmd;
	printf("%s\n", __func__);
	params_dump(params);
	/*TODO: implement the command*/
...
static command_t myzggw_cmds[] = {
	...
	{ STR_NAME_MYZGGW_MOVE_NODE, myzggw_move_node },
	{ STR_NAME_MYZGGW_SET_PAN_CHANNEL, myzggw_set_pan_channel },
	{ STR_NAME_MYZGGW_MANAGE_WHITELIST, myzggw_manage_whitelist_exec },
	{ STR_NAME_MYZGGW_GET_WHITELIST, myzggw_get_whitelist_exec },
	{ STR_NAME_MYZGGW_SET_WHITELIST_ENABLE, myzggw_whitelist_enable_exec },
	{ "set_light", mydev_set_light_exec },//set_light是演示命令，实际项目中可以删除 
	{ NULL, NULL }
};
