...
static void init_devices(ZLG_IoT_Client *mqclient, mqtt_client_t *mq, const char *owner, const char* devtype, const char *devid, const char *secret) {
  ...
  Industry_ES_dev_init(&a_device, owner, devtype, devid, secret);
...
int main(int argc, const char* argv[]) {
  ...
  while (bExit == FALSE) {
    ...
    // 获取模拟的工商储能的统计数据(statistics_data)然后上报到云端，数据格式是key1\0vluae1\0key2\0vluae2\0
    request_init(&params, buff, sizeof(buff));    // 初始化buff内容为空
    Industry_ES_dev_get_statistics_data(&params);
    const char* group_statistics_data = "statistics_data";  // 数据分组对应Industry_ES定义中的数据分组，见云端Web平台的[设备建模]
    ...
    // 获取模拟的工商储能的电池总控(statistics_bms)然后上报到云端，数据格式是key1\0vluae1\0key2\0vluae2\0
    request_init(&params, buff, sizeof(buff));    // 初始化buff内容为空
    Industry_ES_dev_get_statistics_bms(&params);
    const char* group_statistics_bms = "statistics_bms";  // 数据分组对应Industry_ES定义中的数据分组，见云端Web平台的[设备建模]
    if (!client_report_data_multi(&a_device, group_statistics_bms, &params)) {
      // 发送失败后，简单的重发一次
      // 根据实际场景自行决定是否重发，具体重发机制由调用者自行实现：
      //    可以使用队列先保存要重发的内容，后续从队列中读取出来，再次重复
      //    也可以在mqtt_publish中简单的重发若干次
    ...
    // 获取工商储能的逆变器数据(pcs1)然后上报到云端，数据格式是key1\0vluae1\0key2\0vluae2\0
    request_init(&params, buff, sizeof(buff));    // 初始化buff内容为空
    Industry_ES_dev_get_pcs1(&params);
    const char* group_pcs1 = "pcs1";  // 数据分组对应Industry_ES定义中的数据分组，见云端Web平台的[设备建模]
    if (!client_report_data_multi(&a_device, group_pcs1, &params)) {
      // 发送失败后，这里的示例代码，简单的重发一次
      // 根据实际场景自行决定是否重发，具体重发机制由调用者自行实现：
      //    可以使用队列先保存要重发的内容，后续从队列中读取出来，再次重复
      //    也可以在mqtt_publish中简单的重发若干次
    ...
    // Industry_ES_query_strategy为1表示云端要求设备上报设备当前全部策略到云端,
    if(Industry_ES_query_strategy > 0){
      request_init(&params, buff, sizeof(buff));
      params_append_string(&params, "peak_shaving", Industry_ES_peak_shaving_buff); // 削峰填谷策略
      params_append_string(&params, "demand_control", Industry_ES_demand_control);              // 需量控制策略
      //添加更多设备状态
      //params_append_string(&params, "strategy_state", strategy_state);            // 策略总体状态
      //params_append_string(&params, "anti_reflux", anti_reflux);                  // 防逆流策略
      ...
      Industry_ES_query_strategy = 0;
    }else {
      // 当设备状态改变时，设备也可以主动上报设备状态给云端
      // 这里演示程序假设demand_control状态已经改变，主动上报demand_control状态
      request_init(&params, buff, sizeof(buff));
      params_append_string(&params, "demand_control", Industry_ES_demand_control);
      if (!client_report_status(&a_device, &params)) {
        // 发送失败后，这里的示例代码，简单的重发一次
        if (!client_report_status(&a_device, &params)) {
          continue; // 可能网络存在问题，重新调用zlg_iot_mqtt_yield
      ...
      // 这里演示程序假设peak_shaving状态已经改变，主动上报peak_shaving状态
      request_init(&params, buff, sizeof(buff));
      params_append_string(&params, "peak_shaving", Industry_ES_peak_shaving_buff);
      if (!client_report_status(&a_device, &params)) {
        // 发送失败后，这里的示例代码，简单的重发一次
        if (!client_report_status(&a_device, &params)) {
          continue; // 可能网络存在问题，重新调用zlg_iot_mqtt_yield
